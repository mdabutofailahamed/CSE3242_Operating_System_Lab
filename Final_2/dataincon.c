...
#define NUM_ITERATIONS 1000000  // Number of times the counter will be incremented

int counter = 0;               // Global variable
pthread_mutex_t counter_mutex;  // Mutex for protecting the counter

// Function to increment counter without using mutex (data inconsistency)
...
// Function to increment counter with mutex protection (data consistency)
void* increment_with_mutex(void* arg) {
    for (int i = 0; i < NUM_ITERATIONS; i++) {
        pthread_mutex_lock(&counter_mutex);  // Lock the mutex to protect the counter
...
int main() {
    ...
    // Demonstrate data consistency with mutex
